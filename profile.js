...
//各キャストのページで実行された場合、勝率などの情報を取得する
	...
	if (d.getElementById('wlw_custom')==null) {
		...
		var clca = [];
		// 各キャストの対戦数 ... cast battle count array
		var cbca = [];
...
function disp_proc(){
	...
	cwca[ci] = wc[proc_ci];
	clca[ci] = lc[proc_ci];
	cbca[ci] = cwca[ci] + clca[ci];

	//総使用回数でソートするための処理
	var forsort = [];
	for (var i = 0; i < dci.length; i++) {
		forsort[i] = new tbcsort(dci[i], cbca[dci[i]], cwca[dci[i]]);
	}
	...
	for (var i = 0; i < dci.length; i++) {
	// 試合数が29以下のキャストは表示しない
		if ((cwca[forsort[i].id]+clca[forsort[i].id])>29) {
			var cni = dci.indexOf(forsort[i].id);
			insert(6, "<span class=\"font_90\">"+dcn[cni]+"</span>", cwra[forsort[i].id]+"% <span class=\"font_small\">("+cwca[forsort[i].id]+"勝"+clca[forsort[i].id]+"敗)</span>");		
		}
	}
	...
	for(var cnt=0; cnt < dci.length; cnt++){
		var cnti = dci.indexOf(forsort[cnt].id);
		//使用回数29回以下のキャストのボタンは表示しない
		if(wc[cnti]+lc[cnti]>29){
			imgNode_cast[cnti] = d.createElement("img");
			imgNode_cast[cnti].src = CAST_IMG_URL + castimgurl[cnti];

			imgNode_cast[cnti].width = icon_width;
			imgNode_cast[cnti].height = icon_height;
		
			var linkNode = d.createElement("a");
			//linkNode.href = "JavaScript:changedisp(" + cnti.toString() + ")";
			linkNode.appendChild(imgNode_cast[cnti]);
			gameNode.appendChild(linkNode);
			
			//表示ページのキャストは半透明に設定する
			//if(cnti == proc_ci){
			//	//表示ページのキャストは半透明に設定する
			//	imgNode_cast[cnti].style.opacity = 0.5;
			//}
...
function tbcsort(_id, _tbc, _wbc) {
	this.id = _id;
	this.tbc = _tbc;
	this.wbc = _wbc;
}
